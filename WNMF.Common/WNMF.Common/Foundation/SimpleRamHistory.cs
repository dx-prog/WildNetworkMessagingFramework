...
namespace WNMF.Common.Foundation {
    ...
    public class SimpleRamHistory : INetworkMessageHistory {
        ...
        public virtual bool TryCheckIfSent(INetworkEndpoint endpoint,
            NetworkMessageDescription messageDescription,
            out TryOperationResponse<bool> reason) {
            lock (_sent) {
                var key = CreateKey(endpoint, messageDescription);
        ...
        private static string CreateKey(INetworkEndpoint endpoint, NetworkMessageDescription messageDescription) {
            var key = endpoint.GlobalId + ";" + messageDescription.Id;
            return key;
        }
    }
}